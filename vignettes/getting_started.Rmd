...
```r
remotes::install_github('riedam/turbidata', dependencies = TRUE)
```
Une fois installé, ne pas oublier de l'importer
```r
library(turbidata)
```
Controller la version du package :
```r
...
#### gganim
Identique à `$ggplot` mais en version annimé. Attention : le temps de rendu peut être long. Exemples :
```r
test_data$gganim
test_data$average_area(min, max)$gganim
```
